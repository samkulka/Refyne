...
function FileRow({ file, onOpenHealth }) {
  const statusChip = {
...
function DataHealthDialog({ open, onOpenChange, file }) {
...
function ProfilesGrid({ data, onOpenProfile }) {
  return (
        ...
        <Card key={c.id} className="rounded-2xl hover:shadow-md transition">
          <CardContent className="pt-5">
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-3">
                <Avatar className="h-10 w-10"><AvatarFallback>{c.name.split(" ").map(x=>x[0]).join("")}</AvatarFallback></Avatar>
                <div>
...
function ProfileDrawer({ customer, onClose }) {
    ...
    <div className="fixed inset-0 z-50 flex">
      ...
      <div className="w-full max-w-xl bg-background shadow-2xl animate-in slide-in-from-right rounded-l-2xl">
        <div className="p-5 flex items-start justify-between">
          <div className="flex items-center gap-3">
            <Avatar className="h-10 w-10"><AvatarFallback>{customer.name.split(" ").map(x=>x[0]).join("")}</AvatarFallback></Avatar>
            <div>
...
function RelationshipCanvas({ customers }) {
